...
cleanup() {
    if [ -f ".env.backup" ]; then
        echo "🔄 Восстанавливаем .env файл с токенами..."
        mv .env.backup .env
    fi
}

# Устанавливаем обработчик для восстановления .env при выходе
trap cleanup EXIT
...
if [ -n "$(git status --porcelain)" ]; then
    ...
    # Проверяем, есть ли изменения в .env файле
    if git diff --name-only | grep -q "\.env$"; then
        echo "🔒 Исключаем .env файл из коммита (содержит токены)..."
        git checkout -- .env
    fi
    
    # Сохраняем .env файл если он существует
    if [ -f ".env" ]; then
        echo "💾 Сохраняем .env файл с токенами..."
        cp .env .env.backup
    fi
    
    # Добавляем все изменения (кроме .env)
